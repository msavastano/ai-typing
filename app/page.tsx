...
export default function Home() {
  ...
  // Load the Gemini key from the current browser session on mount.
  // sessionStorage is client-only, so this must run after hydration.
  useEffect(() => {
  ...
  if (isTyping) {
    return (
      <div className="flex min-h-screen flex-col bg-[#d9d1c0] text-[#2a2620]">
        <Nav user={user} onSignIn={signIn} onSignOut={handleSignOut} />
        ...
        <div className="max-w-[720px] mx-auto px-6 flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              disabled={!hasKey}
              title={hasKey ? undefined : 'Add your Gemini API key below to start'}
              className="font-serif text-[0.9rem] font-semibold text-white bg-[#665f51] hover:bg-[#3d3830] border-none rounded-md px-5 py-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
            >
              Start AI lesson
            </button>
          </div>
          ...
          {/* Gemini API key */}
          <Card className={`p-5 ${hasKey ? '' : 'border-l-[3px] border-l-[#665f51]'}`}>
            <div className="flex items-center justify-between mb-1">
              <div className="font-serif text-base font-bold text-[#2a2620]">Gemini API key</div>
              {hasKey && (
                <span className="font-serif text-[0.72rem] font-medium text-[#4a7a4a] bg-[#eef4ee] border border-[#d6e3d6] rounded-full px-2.5 py-0.5">
                  Active this session
                </span>
              )}
            </div>
            <div className="font-serif text-[0.78rem] text-[#7b7771] leading-[1.6] mb-3.5">
              AI lessons are generated by Google Gemini. You supply your own key —
              it is held only in this browser session and is cleared when you close the tab or sign out.
              It is never saved to our servers or to disk.
            </div>

            {hasKey ? (
              <button
                onClick={handleClearKey}
                className="font-serif text-[0.8rem] text-[#665f51] bg-[#f1edea] border border-[#dcd9d7] rounded-md px-4 py-1.5 cursor-pointer hover:bg-[#e5e2df] transition-colors"
              >
                Remove key
              </button>
            ) : (
              <>
                <div className="bg-[#f6f3f1] border border-[#e5e2df] rounded-md p-4 mb-3.5">
                  <div className="font-serif text-[0.72rem] font-semibold uppercase tracking-[0.08em] text-[#7b7771] mb-2">
                    How to get a free key
                  </div>
                  <ol className="font-serif text-[0.82rem] text-[#2a2620] leading-[1.7] list-decimal pl-4 space-y-0.5">
                    <li>
                      Open{' '}
                      <a
                        ...
                        className="text-[#665f51] underline font-semibold hover:text-[#3d3830]"
                      >
                        Google AI Studio
                      </a>{' '}
                  ...
                  <div className="font-serif text-[0.75rem] text-[#7b7771] leading-[1.6] mt-3">
                    <strong className="text-[#7a3f3f]">Keep it private.</strong> Anyone with your key can
                    spend against your Google account. Usage may count toward Gemini API limits or billing on
                    your account — review the free-tier limits in AI Studio before heavy use.
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-md bg-[#f6f3f1] border border-[#dcd9d7] text-[#2a2620] text-sm placeholder-[#b5b0aa] focus:outline-none focus:border-[#665f51] font-mono"
                  />
                  <button
                    onClick={handleSaveKey}
                    disabled={!keyInput.trim()}
                    className="font-serif text-[0.85rem] font-semibold text-white bg-[#665f51] hover:bg-[#3d3830] border-none rounded-md px-5 py-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
                  >
                    Save key
                  </button>
                </div>
            ...
            {keySaved && (
              <div className="font-serif text-[0.78rem] text-[#4a7a4a] mt-2">Key saved for this session.</div>
            )}
          </Card>
